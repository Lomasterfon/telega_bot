...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, "Що потрібно?", reply_markup=admin_kb.button_case_admin)
    await message.delete()


# Start download new category
# @dp.message_handler(commands='Завантажити', state=None)
async def cm_start(message: types.Message):
    if message.from_user.id == ID:
        await FSMAdmin.photo.set()
        await message.reply('Завантаж картинку')
...
# Catch first answer and write list
...
# Catch second answer
# @dp.mesage_handler(state: FSMAdmin.name)
